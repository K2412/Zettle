...
export async function fetchConnectCandidates(
    noteSlug: string,
): Promise<ConnectCandidate[]> {
    const data = await postJson<{ candidates?: ConnectCandidate[] }>(
        ConnectController.run.url(noteSlug),
    );

    return data?.candidates ?? [];
}

export async function fetchMakeFindable(
    noteSlug: string,
): Promise<MakeFindableResult | null> {
    return postJson<MakeFindableResult>(MakeFindableController.run.url(noteSlug));
}

export async function fetchStructure(
    noteSlug: string,
): Promise<StructureResult | null> {
    return postJson<StructureResult>(StructureController.run.url(noteSlug));
}

export async function fetchClusterProject(
    noteSlug: string,
): Promise<ClusterProjectResult[]> {
    const data = await postJson<{ clusters?: ClusterProjectResult[] }>(
        ClusterProjectController.run.url(noteSlug),
    );
